...
class childComponent extends React.Component {
  ...
  componentWillMount() {
    ...
    $.ajax({
      ...
      success: function(data) {
        for (let i in data) {
          if (i !== null) {
            if(data[i].flag.low == 1){//console.log(JSON.stringify(data));
  ...
  this.props.domainName2(this.state.searchQuery);
    ...
    // $.ajax({
    //   url:"/users/findCompletedScenarios",
    //   type:'POST',
    //   data:{empId:cookies.get('empId')},
    //   success: function(data)
    //   {
    //     data.completedScenario.map(function (item){
    //       if(item.domainName == domainName1){
    //         item.scenarioIds.map((item)=>{
    //           arr.push(item.id);
    //         })
    //       }
    //     })
    //     this.props.buttonStatus(arr);
    //   }.bind(this),
    //   error: function(err)
    //   {
    //     //console.log('error occurred on AJAX');
    ...
    if(this.state.domain.length != 0){
      domainPage = (<div>
        ...
        <center id = "landing">
          ...
          <div >
            <Dropdown onChange={this.updatesearchQuery.bind(this)} placeholder='Select Customer Journey' style={{fontFamily:'Arial'}} search selection options={this.state.domain}></Dropdown>
            <Button primary onClick={this.handleSubmit.bind(this)} style={{marginLeft:'1%',fontSize:'101%'}} content="Select"/>
            {this.state.result.length>0
              ?<div>
                <Grid divided='vertically'>
                <Grid.Row>
                  <Grid.Column width={4}></Grid.Column>
                  <Grid.Column width={7} style={{marginTop:'11px', marginBottom:"-3%"}}>
                    <Scrollbars renderTrackHorizontal={props => <div {...props} className="track-horizontal" style={{
                      display: 'none',
                      position: 'right'
                    }}/>} autoHeight autoHide>
                    <div style={{height:"85px",marginTop:"1%"}} id="domainDescriptionHeight">
                      {this.state.result}
                      </div>
                </Scrollbars>
                    {/* <Button style={{float:"right"}}>Proceed</Button> */}
                  </Grid.Column>
                  <Grid.Column>
            ...
            <Grid.Row>
              <Grid.Column width={4}></Grid.Column>
              <Grid.Column width={7} style={{marginTop:"2%", textAlign: "left", lineHeight: "1.9em", fontFamily:"arial", marginBottom:"-3%"}}>
                <p style={{marginTop:"2%", textAlign: "justify", lineHeight: "1.8em"}}><b>Max Health app</b> provides a portal that connects the patients and the phlebotomists directly and make it easy for booking, tracking and recording of the appointments. The application also provides managerial and organizational facilities to the hospital. There are four types of users for the app with different functionalities- <b>Customer Care, Patient, Coordinator and Phlebotomist</b>.</p>
                <p style={{marginTop:"2%", textAlign: "justify", lineHeight: "1.8em"}}>Each of these users are inter-connected and are required to send information to each other for completion of a successful transaction. For example, a new user needs to call customer care for registration and book an appointment. Once appointment is booked, the information is sent to the coordinator. Coordinator then allocates a phlebotomist to the appointment who will get the necessary data and the same will be communicated to the patient.</p>
              </Grid.Column>
              <Grid.Column>
                <div className='checking'><iframe style={{width:"273px",marginTop:"26%"}} src="https://www.youtube.com/embed/NyfSE5NWMuM?rel=0&amp;showinfo=0" frameBorder="0" allowFullScreen></iframe></div>
            </Grid.Column>
            </Grid.Row>
          </Grid>
          }
      ...
      //   Loading content... <br/>If not loaded in 5 seconds Please Reload to continue...
      // </div>
      <div><Card  color="red" style = {{margin:"auto",marginTop:'100px'}}>
         <Card.Content>
